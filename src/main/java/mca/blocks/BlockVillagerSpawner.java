...
public class BlockVillagerSpawner extends Block {
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random random) {
        ...
        // 检查生成器上限和村庄人口上限
        boolean spawnerCapReached = nearbyVillagers >= MCA.getConfig().villagerSpawnerCap;
        boolean populationCapReached = mca.core.minecraft.VillageHelper.isVillageAtPopulationCap(world,
                world.getVillageCollection().getNearestVillage(pos, 64));

        if (!spawnerCapReached && !populationCapReached) {
            int yMod = 0;
